...
def top_titles
  ...
  Movie.select("movies.id, movies.title").where("score >= '9.0'")
...
def below_average_years
  ...
  Movie.select('movies.yr, COUNT(*) AS bad_movies').where("movies.score < '5.0'").group('movies.yr').order('bad_movies DESC')
...
def alphabetized_actors
  ...
  Actor.select('actors.id, actors.name').order('actors.name ASC').limit(10)
